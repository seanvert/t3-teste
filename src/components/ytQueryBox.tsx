...
export function YTQueryBox () {
	...
	async function handleChange(key) {
		setQuery(key)
		setKeywordString(query.replace(/\s/g, "+"));
		/* var url = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=5&q=${keywordString}&type=video&key=${process.env.YOUTUBE_API_KEY}` */
		
		/* var url = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=20&q=${keywordString}&type=video&key=AIzaSyCbzxPzZnXq2Lx8gI62uI1LQArHjtPiks4` */
		/* console.log(url); */
